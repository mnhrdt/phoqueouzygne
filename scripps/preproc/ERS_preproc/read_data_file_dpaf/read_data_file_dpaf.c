...
int	file_size,nitems,year;
int	prior_pri_dn,prior_swst_dn;
int 	endian;
FILE	*indata,*logfile;
...
/* read top of file */
nitems = fread(&slb,sizeof(struct sarleader_binary),1,indata);
...
/* nitems = fread(&sdr,sizeof(struct sardata_rec),1,indata); */
 nitems = fread(&dpaf_sdr,sizeof(struct sardata_dpaf_rec),1,indata);
