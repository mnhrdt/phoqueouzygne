...
void do_correlation(struct xcorr xc)
{
	...
	for (i=0; i<xc.nyl; i+=istep){
		...
		for (j=0; j<xc.nxl; j++){
			...
			if (xc.corr_flag == 2) do_freq_corr(xc, iloc); 

			/* oversample correlation surface  to obtain sub-pixel resolution */
			if (xc.interp_flag == 1) do_highres_corr(xc, iloc); 
...
/*-------------------------------------------------------------------------------*/
void assign_values(struct xcorr xc, int iloc)
{
int	i, j, k, sx, mx;
double	mean1, mean2;
	...
	if (xc.ri > 1) {
		for (i=0; i<xc.npy; i++) {
			do_range_interpolate(&xc.c1[i*xc.npx], xc.npx, xc.ri, xc.ritmp);
			do_range_interpolate(&xc.c2[i*xc.npx], xc.npx, xc.ri, xc.ritmp);
			}
		}
	...
	for (i=0; i<xc.npy*xc.npx; i++) {
		xc.c1[i].r = xc.c1[i].r - mean1;
		xc.c2[i].r = xc.c2[i].r - mean2;
		}

	/* apply mask */
	for (i=0; i<xc.npy*xc.npx; i++) {
			xc.c1[i].i = xc.c2[i].i = 0.0;
...
/*-------------------------------------------------------------------------------*/
int do_range_interpolate(struct FCOMPLEX *c, int nx, int ri, struct FCOMPLEX *work)
{
int	i;

	/* interpolate c and put into work */
	fft_interpolate_1d(c, nx, work, ri);
