...
do
  ...
  if [ "$ac" -eq 0 ]; then
    ...
    grdmath $grd sum.grd ADD = tmp.grd 
    mv tmp.grd sum.grd  
  fi
...
grdmath sum.grd $ac DIV 365 MUL $wavel MUL -79.58 MUL = $outmean
...
do
  if [ "$num" -eq 0 ]; then
    grdmath $grd ${dayarray[$num]} DIV 365 MUL $wavel MUL -79.58 MUL $outmean SUB SQR = sum2.grd 
  else 
    grdmath $grd ${dayarray[$num]} DIV 365 MUL $wavel MUL -79.58 MUL $outmean SUB SQR sum2.grd ADD = sum2tmp.grd
    mv sum2tmp.grd sum2.grd
  fi
  let "num += 1"
done 
grdmath sum2.grd $num DIV SQRT = $outstd
