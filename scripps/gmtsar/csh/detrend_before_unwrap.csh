...
#set rng_samp = `grdinfo -I corr.grd | sed -e 's:I:/:' | awk -F/ '{print $2}'`
...
#set az_samp = `grdinfo -I corr.grd | sed -e 's:I:/:' | awk -F/ '{print $3}'`
...
set rng_actual_min = `grdinfo -I- unwrap.grd | sed -e 's:R:/:' | awk -F/ '{print $2}'`
set rng_actual_max = `grdinfo -I- unwrap.grd | sed -e 's:R:/:' | awk -F/ '{print $3}'`
set az_actual_min  = `grdinfo -I- unwrap.grd | sed -e 's:R:/:' | awk -F/ '{print $4}'`
set az_actual_max  = `grdinfo -I- unwrap.grd | sed -e 's:R:/:' | awk -F/ '{print $5}'`
...
grdedit unwrap.grd -R$near_range_crop/$far_range_crop/$azi_start_crop/$azi_end_crop
grd2xyz unwrap.grd -S > unwrap.dat
...
foreach n ( 1 2 3 4 5 )
  ...
  set far_range = `echo $near_range $numrng $cee $rng_samp_rate |awk '{printf("%.14f", $1 + $2*$3/(2.0*$4))}'`
  #set rng_samp = `grdinfo -I corr.grd | sed -e 's:I:/:' | awk -F/ '{print $2}'`
  #set drng = `echo $far_range $near_range $numrng $rng_samp |awk '{printf("%.14f", $4*($1-$2)/$3)}'`

  #compute start time relative to the start time of the selected frame, in seconds
  #azi_time(n,j) = (SC_clock_start(n) - SC_clock_start(1)) + j / PRF(n)
  ...
  #set az_samp = `grdinfo -I corr.grd | sed -e 's:I:/:' | awk -F/ '{print $3}'`
  #set daz = `echo $azi_end $azi_start $numaz $az_samp |awk '{printf("%.14f", $4*($1-$2)/$3)}'`

  # edit the coordinates
  echo "edit coordinates to -R$near_range/$far_range/$azi_start/$azi_end"
  grdedit phasefilt_nodetrend.grd -R$near_range/$far_range/$azi_start/$azi_end

  # subtract the fitted trend
  grdmath -V phasefilt_nodetrend.grd X $rslope MUL SUB Y $aslope MUL SUB 2 PI MUL MOD PI SUB = phasefilt.grd

  # edit the coordinates back
  grdedit phasefilt.grd -Rcorr.grd
  grdedit phasefilt_nodetrend.grd -Rcorr.grd
