...
void put_sio_struct(struct PRM prm, FILE *OUTFILE)
{
	...
	if (prm.fs != NULL_DOUBLE) fprintf(OUTFILE,"rng_samp_rate   		= %.6lf \n",prm.fs);
	...
	if (prm.lambda != NULL_DOUBLE) fprintf(OUTFILE,"radar_wavelength	= %lf \n",prm.lambda);
	if (prm.chirp_slope != NULL_DOUBLE) fprintf(OUTFILE,"chirp_slope	= %lg \n",prm.chirp_slope);
	if (prm.fs != NULL_DOUBLE) fprintf(OUTFILE,"rng_samp_rate		= %.6f \n",prm.fs);
	if (prm.xmi != NULL_DOUBLE) fprintf(OUTFILE,"I_mean			= %lf \n",prm.xmi);
	if (prm.xmq != NULL_DOUBLE) fprintf(OUTFILE,"Q_mean			= %lf \n",prm.xmq);
